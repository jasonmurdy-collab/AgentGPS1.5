...
import type { DailyTrackerData, HabitTrackerTemplate, HabitActivitySetting, ProspectingSession, TeamMember } from '../types';
...
// --- DEFAULT DATA & HELPERS FOR COACH ---
const defaultCoachHabitActivities: HabitActivitySetting[] = [
    { id: 'recruitingCalls', name: 'Recruiting Calls', worth: 2, unit: 'call' },
    { id: 'recruitingAptsSet', name: 'Recruiting Appts Set', worth: 10, unit: 'appt' },
    { id: 'recruitingAptsHeld', name: 'Recruiting Appts Held', worth: 20, unit: 'appt' },
    { id: 'newAgentsSigned', name: 'New Agents Signed', worth: 50, unit: 'agent' },
    { id: 'agentCoachingSessions', name: '1-on-1 Coaching Sessions', worth: 15, unit: 'session' },
    { id: 'agentGoalReview', name: 'Agent Goal Review Sessions', worth: 10, unit: 'review' },
    { id: 'groupTrainingConducted', name: 'Group Training Conducted', worth: 20, unit: 'session' },
    { id: 'agentFollowUps', name: 'Agent Follow-up Calls', worth: 1, unit: 'call' },
    { id: 'reviewAgentPerformance', name: 'Review Agent Performance', worth: 5, unit: 'review' },
    { id: 'teamMeetingPrep', name: 'Team Meeting Prep (Hours)', worth: 5, unit: 'hour' },
    { id: 'trainingMaterialCreation', name: 'Training Material (Hours)', worth: 10, unit: 'hour' },
    { id: 'leadershipMeetings', name: 'Leadership Meetings', worth: 5, unit: 'meeting' },
    { id: 'socialRecruitingPosts', name: 'Social Recruiting Posts', worth: 3, unit: 'post' },
    { id: 'networkingEventAttended', name: 'Networking Event Attended', worth: 10, unit: 'event' },
];

const getInitialCoachTrackerData = (userId: string, date: Date, userData: TeamMember | null): Omit<DailyTrackerData, 'id'> => {
    ...
    return {
        ...
        pointsActivities: {}, // Coach activities will be stored here
...
const PrintableTracker: React.FC<{ data: DailyTrackerData; settings: HabitTrackerTemplate; userName: string; }> = ({ data, settings, userName }) => {
    ...
    return (
        <div id="pdf-export-content-coach" className="p-10 bg-white text-gray-800 w-[827px] font-sans">
            <div className="text-center mb-6 border-b-2 border-[var(--color-accent)] pb-4">
                <h1 className="text-3xl font-bold text-[var(--color-accent)]">Coach Daily Accountability Report</h1>
            ...
            <div className="grid grid-cols-3 gap-4 text-center mb-6 bg-gray-100 p-4 rounded-lg">
                ...
                <div><p className="text-sm font-semibold text-gray-600">Recruiting Appts</p><p className="text-3xl font-bold">{data.pointsActivities?.recruitingAptsSet || 0}</p></div>
            </div>
...
const PrintableAllLogs: React.FC<{ logs: DailyTrackerData[]; settings: HabitTrackerTemplate; userName: string; }> = ({ logs, settings, userName }) => {
    ...
    return (
        <div id="pdf-export-all-content-coach" className="bg-white text-gray-800 w-[827px] font-sans">
    ...
    return (
        <div className="p-4 bg-background/50 border-t border-border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center mb-4">
                ...
                <div className="bg-surface p-2 rounded-lg"><p className="text-xs text-text-secondary">Recruiting Calls</p><p className="font-bold text-lg">{log.pointsActivities?.recruitingCalls || 0}</p></div>
                <div className="bg-surface p-2 rounded-lg"><p className="text-xs text-text-secondary">Recruiting Appts</p><p className="font-bold text-lg">{log.pointsActivities?.recruitingAptsSet || 0}</p></div>
                <div className="bg-surface p-2 rounded-lg"><p className="text-xs text-text-secondary">Coaching Sessions</p><p className="font-bold text-lg">{log.pointsActivities?.agentCoachingSessions || 0}</p></div>
            </div>
                ...
                <div>
                    ...
                    <ul className="list-disc list-inside text-text-secondary space-y-1">
                        {(settings.activities || []).map(act => {
                            const value = log.pointsActivities?.[act.id] || 0;
...
// --- MAIN PAGE COMPONENT ---
const CoachDailyHabitsTrackerPage: React.FC = () => {
    ...
    useEffect(() => {
      const fetchSettings = async () => {
        if (!user) {
          setHabitSettings({ id: 'fallback', name: 'Default Coach', activities: defaultCoachHabitActivities });
          return;
        }
        ...
        const loadTrackerData = async () => {
            ...
            try {
                ...
                if (!querySnapshot.empty) {
                    ...
                    setDocId(null);
                    loadedData = getInitialCoachTrackerData(user.uid, currentDate, userData) as DailyTrackerData;
                    setTrackerData(loadedData);
                }
                ...
                setDocId(null);
                const initialData = getInitialCoachTrackerData(user.uid, currentDate, userData) as DailyTrackerData;
    ...
    const handleExportPdf = () => {
        ...
        setExporting(true);
        const reportElement = document.getElementById('pdf-export-content-coach');
        if (reportElement) {
    ...
    const handleExportAllLogs = () => {
        ...
        const reportElement = document.getElementById('pdf-export-all-content-coach');
        if (reportElement) {
            html2canvas(reportElement, { scale: 2, useCORS: true }).then(canvas => {
                const imgData = canvas.toDataURL('image/png');
                const pdf = new jsPDF('p', 'px', 'a4');
    ...
    return (
        <div className="h-full flex flex-col">
            <header className="p-4 sm:p-6 lg:p-8">
                <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-text-primary flex items-center gap-3"><UserSearch/> My Daily Tracker</h1>
                <p className="text-lg text-text-secondary mt-1">Track your high-leverage coaching and recruiting activities.</p>
                ...
                {activeTab === 'tracker' ? (
                    ...
                    <Card>
                         <div className="flex justify-between items-center mb-4">
                            ...
                            {allLogs.length > 0 && <button onClick={handleExportAllLogs} disabled={exportingAll} className="flex items-center justify-center bg-accent text-on-accent font-semibold py-2 px-4 rounded-lg disabled:opacity-50">{exportingAll ? <Spinner /> : <><Download className="mr-2" size={16} /> Export All Logs</>}</button>}
                        </div>
                        {loadingLogs ? <div className="flex justify-center items-center py-8"><Spinner/></div> : allLogs.length > 0 ? (
                                ...
                                {allLogs.map(log => {
                                    ...
                                    return (
                                        <Card key={log.id} className="p-0 overflow-hidden">
                                            <button onClick={() => setExpandedLogId(isExpanded ? null : log.id)} className="w-full flex justify-between items-center p-4 text-left hover:bg-primary/5">
                            ...
                            <p className="text-center text-text-secondary py-8">You have no saved logs yet.</p>
                        )}
                    </Card>
                )}
            </div>
             <div className="absolute -left-[9999px] -top-[9999px] opacity-0" aria-hidden="true">
                <PrintableTracker data={trackerData} settings={habitSettings} userName={userData?.name || 'Coach'} />
                {activeTab === 'logs' && allLogs.length > 0 && <PrintableAllLogs logs={allLogs} settings={habitSettings} userName={userData?.name || 'Coach'} />}
            </div>
        </div>
    );
};

export default CoachDailyHabitsTrackerPage;